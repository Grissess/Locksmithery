...
public class SecureBlockTE extends TileEntity implements IBittedTE {
    public LocksetBitting bitting;

    public SecureBlockTE() {
        this(BittingDescriptor.WOOD);
    }
    public SecureBlockTE(BittingDescriptor desc) {
        bitting = new LocksetBitting(desc);
    }
    public SecureBlockTE(LocksetBitting bit) {
        bitting = bit;
    }
